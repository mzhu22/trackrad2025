...
#SBATCH --output=nnUNet-train-slurm-1.out
#SBATCH --error=nnUNet-train-slurm-1.err
...
CUDA_VISIBLE_DEVICES=0 nnUNetv2_train "$DATASET" "$CONFIG" 0 -p "$PLAN" --c --npz -tr "$TRAINER" &
CUDA_VISIBLE_DEVICES=1 nnUNetv2_train "$DATASET" "$CONFIG" 1 -p "$PLAN" --c --npz -tr "$TRAINER" &
CUDA_VISIBLE_DEVICES=2 nnUNetv2_train "$DATASET" "$CONFIG" 2 -p "$PLAN" --c --npz -tr "$TRAINER" &
CUDA_VISIBLE_DEVICES=3 nnUNetv2_train "$DATASET" "$CONFIG" 3 -p "$PLAN" --c --npz -tr "$TRAINER" &
