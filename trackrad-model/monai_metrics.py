...
class IterationMetric:
    ...
    def _compute_list(
        ...
        if y is not None:
            ret = [
                self._compute_tensor(
                    p.detach().unsqueeze(0), y_.detach().unsqueeze(0), **kwargs
                )
                for p, y_ in zip(y_pred, y)
            ]
        else:
            ret = [
                self._compute_tensor(p_.detach().unsqueeze(0), None, **kwargs)
                for p_ in y_pred
            ]
        ...
        if isinstance(ret[0], (list, tuple)) and all(
            isinstance(i, np.ndarray) for i in ret[0]
        ):
            return [np.concatenate(batch_i, axis=0) for batch_i in zip(*ret)]
        return ret

    def _compute_tensor(
        self, y_pred: np.ndarray, y: np.ndarray | None = None, **kwargs: Any
    ) -> TensorOrList:
        """
...
class HausdorffDistanceMetric(IterationMetric):
    ...
    def _compute_tensor(
        ...
        assert y_true.shape == y_pred.shape, (
            f"y_pred and y_true should have same shapes, got {y_pred.shape} and {y_true.shape}."
        )
        ...
        for b, c in np.ndindex(B, C):
            ...
            distances = scipy_distance_transform_edt(
                input=(~edges_gt), sampling=self.spacing
            )
            ...
            if not self.directed:
                distances2 = scipy_distance_transform_edt(
                    input=(~edges_pred), sampling=self.spacing
                )
